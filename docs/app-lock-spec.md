...
| 再ロック / セッション | **解錠セッション方式**。最後の操作から 5 分有効で、操作ごとに延長。セッション中はバックグラウンド復帰でも再認証しない（§6） | 当初は「即ロック（猶予なし）」だったが、通知センターを引いただけ・ファイルピッカーを開いただけで再認証が走る負担が大きく、方針を変更した |
...
解錠したら**セッション**が始まる。セッションは**最後の操作から 5 分**有効で（`SESSION_TIMEOUT_MS`）、**操作のたびに延長**される。セッション中はバックグラウンドへ回して戻っても再認証を求めない。

実装は `src/hooks/useAppSession.ts`。解錠中（`UNLOCKED`）の間だけ購読する。
...
`hidden` へ遷移した時点ではロックしない（セッションを生かすため）。これが「即ロック」からの本質的な変更点。

### 6.2 操作とみなすイベント

`pointerdown` / `keydown` / `wheel` / `touchstart` を capture + passive で購読する。延長は 1 秒間引き（連続入力のたびに store を書き換えない）。`lastActivityAt` はどのコンポーネントも購読していないので、更新しても再描画は起きない。

### 6.3 スライドショー中の扱い

**表示中に再生している間はセッションを延長する**（ポーリングのたびに `touchSession`）。再生は無人で進むため操作イベントが発生せず、何もしないと再生開始から 5 分で必ずロックが落ちる（このアプリの主用途が壊れる）。

ここで「期限チェックを飛ばす」だけでは不十分。ロックはされなくなるが `lastActivityAt` が古いまま残るので、**再生を止めた直後の 1 回目のポーリングで期限切れと判定されて即ロックする**（30 分再生して止めたら 10 秒でロック）。延長でなければならない。

**延長は「表示中」に限る。** バックグラウンドでも延長すると、再生したまま伏せて放置した端末が永久に解錠されたままになり、復帰時の期限判定（§6.1）も意味を失う。

**復帰時の期限判定は再生中でも例外にしない。** バックグラウンドに長く置かれたなら、再生中であってもロックする。ロック時はスライドショーを停止する（解錠時に `AppMain` が再マウントして勝手に再生が再開するのを防ぐ）。
...
### 6.5 セキュリティ上のトレードオフ（明示）

即ロックからセッション方式へ変えたことで、**バックグラウンドへ回した直後の 5 分間はアプリが解錠されたまま**になる。この間に端末を他人が手に取れば中身が見える。§1 の脅威モデル（ロック解除済みの端末を他人が手に取る）に対する防御が、その 5 分ぶん弱くなっている。UX を優先した意図的な判断であり、短くしたい場合は `SESSION_TIMEOUT_MS` 1 箇所で変えられる。

### 6.6 隠せないもの

iOS はバックグラウンド移行時にアプリスイッチャー用のスクリーンショットを撮るが、その撮影は `hidden` の発火や React の再描画より前になり得る。ネイティブの `applicationWillResignActive` 相当が Web には存在しないため、**原理的に防げない**。セッション方式では `hidden` でロックしないので、この点はそもそも変わらない。
...
- **キーサイズは CSS 変数 + `max-height` のメディアクエリで縮む。** manifest が `orientation: landscape` なのでスマホでは viewport の高さが 400px 前後しかなく、固定 68px（4 行 = 314px）だとタイトルや解錠ボタンが画面外へ押し出される。加えてオーバーレイ側を `overflowY: auto` にして、縮めても入り切らない端末で操作不能にならないようにしている
...
| `src/hooks/useAppSession.ts` | 解錠セッションの管理（無操作 5 分・操作で延長） |
