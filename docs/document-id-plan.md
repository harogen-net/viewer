...
> ステータス: **実装済み**（2026-08-28 合意 → 同日実装）。
> 本書は実装前の合意内容を残した設計記録。実装時に判明した事項は末尾の「実装メモ」にある。
...
計画どおりに収まったが、書いていなかった点が 4 つ出た。

1. **HVD の version は 3.2 とし、`docId` は `version` の直後に書く。**
   既存の `imageNames` が 3.1 を使っていたため、両方を持つ文書で version が 3.1 へ
   **下がらないよう** `Math.max` で解決する（version は「含まれる最上位の拡張」を表す単一の値）。
   `docId` を持たない文書はキー自体を出力しないので、既存ファイルとの byte-equal は崩れない。
2. **`markSaved` に docId を渡す必要がある。**
   未保存文書の初回保存と「別名で保存」では docId が新規採番されるため、meta へ書き戻さないと
   次の上書き保存が別レコードを作ってしまう。`markSaved(title, docId?)` に拡張した。
3. **移行時の docId は「本体 HVD が持っていればそれを採用」。**
   同期経由や手動インポートで docId 付き HVD が既に入っている場合に ID を作り直さないため。
   判定は full parse せず正規表現の部分抽出で行う（数 MB の JSON を全件 parse しない）。
4. **本体が欠けた壊れたレコードも一覧だけは移す。**
   `slideData` が無く `slideTitles` にだけ残っているレコードを移行時に捨てると、ユーザーには
   「移行で文書が消えた」に見える。一覧だけ移してロード時に notfound にする。

`useStorage` の API 名も変わった（`listTitles`/`loadByTitle`/`deleteByTitle` →
`listDocs`/`loadById`/`deleteById`、`save` の戻り値が `{docId, title}`）。

## 設計変更: 移行を承認制にする（2026-08-28）

§3.2 では「DB を開いた後に自動で移す」としていたが、**自動移行をやめて二段階の確認を挟む**
形へ変えた。理由は 3 つ。

1. **取り消せない操作を、同意なく、黙って実行していた。** IndexedDB は version を下げられない
   ため、一度実行すると旧バージョンのアプリではデータストアを開けなくなる。
2. **バックアップの機会が無かった。** 移行後に「壊れたら書き出しから戻して」と言っても遅い。
3. **一覧が出るまで待たされていた。** 最初の `listDocs()` が全件の移行完了を待つ作りで、
   進捗表示も無かった（件数と容量によっては起動直後に無言の待ちが発生する）。

### 変更後の流れ

1. 起動時に `probeLegacyDocs()` で**調べるだけ**。`indexedDB.open` を **version 指定なし**で
   呼ぶのが肝で、現行 version のまま開くので `onupgradeneeded` が走らない
   = **スキーマに一切触れない**。
2. 旧形式があれば `LegacyMigrationModal` が確認を **2 回**出す（説明と警告 → 最終確認）。
   バックアップの実行自体はユーザーに任せ、アプリ側では代行しない（警告のみ）。
3. 承認して初めて `runLegacyMigration()` が走る。ここで初めて version 3 へ上がる。進捗表示あり。

**承認前はストレージ API 自体を不活性にする**（`openDbReady()` が null を返し、
`listDocs` は空配列、`save` は null）。中途半端に触らせて version を上げてしまうと、
「まだ何も変わっていないので旧版に戻って書き出す」という退路を塞いでしまうため。
「後で」を選んだ場合、その起動ではドキュメントを開けないまま操作を続けることになる。

状態は `src/state/migrationStore.ts`（`MigrationStatus`: unknown / none / pending / running /
done / failed）。**この状態が ready になった時点で一覧を引き直す**必要がある
（probe は非同期なので、`FileIOToolbar` の初回 effect は空振りする）。

### v4（旧ストアの削除）は据え置き

期限は切らない。移行ロジックは残り続けるが、コード量は小さく、いつでも消せる。
消した時点で未移行の端末のデータは到達不能になるため、急ぐ理由が無い。
