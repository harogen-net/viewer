...
### 2.3 画面向き
- 横起動
- 縦起動（スライドショーのオーバーレイ回転経路。mode-spec §4.2）
...
### 4.3 画面向き
- 縦起動時、スライドショーのオーバーレイが 90° 回転して横画面で表示される
- 回転中に `env(safe-area-inset-*)` が回転後の座標系へ入れ替わり、ノッチ端末で UI が欠けない
- resize 後に viewport 寸法が読み直される（回転中は width/height を swap）
- 一覧 / 編集画面は端末の向きのまま（全体フォールバックは未実装。mode-spec §4.3）
