...
| 項目 | 種別 | 優先 |
|---|---|---|
| stale コメント5件（削除済み legacy パス参照）の整理 | 清掃 | 低 |
| `css/index.css`（legacy `#wrapper` 用の可能性）の要否確認・除去 | 清掃 | 低 |
| roadmap-v4 の LOC KPI（< 6,500）を実態に合わせ改訂 | ドキュメント | 中 |
| mobile PWA モード + 画面向き（net-new、未実装） | 新規機能 | 別途 |
| センシティブモードのパスワードゲート + SHA 暗号化（フラグのみ実装） | 新規機能 | 別途 |
| UI 改善（実運用で「余地あり」との評価） | 改善 | 別途 |
| リモートへの push | 運用 | — |

（mobile / sensitive は「移行」ではなく「今後の新規実装」。完了判定の対象外。）
