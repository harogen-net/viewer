...
export class Viewer {
  ...
  public static startUpMode: ViewerStartUpMode =
    ViewerStartUpMode.VIEW_AND_EDIT;

  //スライドのサイズ基本値として必要
  public static readonly SCREEN_WIDTH = Math.max(
    window.screen.width,
    window.screen.height,
  );
  public static readonly SCREEN_HEIGHT = Math.min(
    window.screen.width,
    window.screen.height,
  );
  ...
  constructor(
    ...
    this.documentStorage = new DocumentStorageUseCase(
      createStorageAdapter(),
      this.featureGate,
    );
    this.documentStorage.addEventListener(
      StorageEventType.LOADING,
      (e: CustomEvent) => {
    ...
    if (startUpMode == ViewerStartUpMode.VIEW_AND_EDIT) {
      HistoryManager.init();
      HistoryManager.shared.addEventListener(
    ...
    //IO section
    {
      ...
      if (startUpMode == ViewerStartUpMode.VIEW_AND_EDIT) {
        ...
        $(".export").click(() => {
          ...
          if (this.listVC.slides.length > 0) {
            ...
            const result = this.documentStorage.exportResult(
              this.viewerDocument,
              type,
              {
                pages:
                  this.listVC.selectedSlideIndex != -1
                    ? [this.listVC.selectedSlideIndex]
                    : undefined,
              },
            );
            if (result.ok === false) {
      ...
      $(".startSlideShow").click(() => {
        ...
        for (var i: number = 0; i < this.viewerDocument.slides.length; i++) {
          ...
          if (i == this.listVC.selectedSlideIndex)
      ...
      $(".save").click(() => {
        ...
        const result = this.documentStorage.saveResult(
          this.viewerDocument,
          isOverride,
        );
        if (result.ok === false) {
