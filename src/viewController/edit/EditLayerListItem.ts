...
export class EditLayerListItem {
	...
	private update(flag: number = PropFlags.ALL) {
		...
		if (flag & PropFlags.IMG_IMAGEID) {
			this.thumbnail.attr(
				"src",
				ImageManager.shared.getSrcById(((this._layerView as ImageView).data as ImageLayer).imageId)
			);
		}
	...
	public set layerView(value: LayerView) {
		if (this._layerView) {
			...
			this._layerView.data.removeEventListener(
				PropertyEvent.UPDATE,
				this.onLayerAndLayerViewUpdate
			);
		}
