...
import { SlideVCWrapper } from "../components/list/SlideVCWrapper";
...
export class ListViewController extends EventDispatcher implements IDroppable {
	...
	private _selectedSlide: Slide | undefined;

	private _mode: ViewerMode = ViewerMode.SELECT;
	...
	constructor(public obj: any) {
		...
		if (Viewer.startUpMode == ViewerStartUpMode.VIEW_AND_EDIT) {
			...
			dropHelper.addEventListener(DropHelper.EVENT_DROP_COMPLETE, (e: CustomEvent) => {
				...
				var slide = new Slide(undefined, undefined, [layer]);
    ...
    ReactRoot.getRoot("reactlist").render(
			<SlideVCWrapper mode={ViewerMode.EDIT} slides={this._slides} />
		);
	...
	private getSlideViewBySlide(slide: Slide | undefined): ThumbSlideView | undefined {
	...
	clonseSlide(slide: Slide): Slide | undefined {
		if (this._slides.indexOf(slide) == -1) return undefined;
	...
	removeSlide(slide: Slide, isAnimation: boolean = false): Slide | undefined {
		var index: number = this._slides.indexOf(slide);
		if (index == -1) return undefined;

		var slideView = this.getSlideViewBySlide(slide);
		...
		var nextSlide: Slide | undefined = undefined;
	...
	private selectSlide(slide: Slide) {
	...
	private selectSlideOffset(offset: number = 0) {
		if (offset == 0) return;
		if (this._selectedSlide == undefined) return;
	...
	// getset
	//
	get selectedSlide(): Slide | undefined {
		return this._selectedSlide;
	}
	get selectedSlideView(): SlideView | undefined {
