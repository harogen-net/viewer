...
export class DOMSlideView extends SlideView {
	...
	set scale(value: number) {
		this._scale =
			value > this.scale_min ? (value < this.scale_max ? value : this.scale_max) : this.scale_min;
		...
		var defX =
			-((this._slide.width * (1 - actualScale)) / 2) + (this.obj.width() - containerWidth) / 2;
		var defY =
			-((this._slide.height * (1 - actualScale)) / 2) + (this.obj.height() - containerHeight) / 2;

		this.container.css(
			"transform",
			"matrix(" + actualScale + ",0,0," + actualScale + "," + defX + "," + defY + ")"
		);
