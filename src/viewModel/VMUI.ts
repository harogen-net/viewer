...
export class VMToggleButton extends VMUI {
	constructor(
		protected _obj: any,
		targetClass: any,
		private targetPropKey: string,
		private targetPropFlag: number
	) {
		super(_obj, targetClass);
		if (_obj.prop("tagName") != "BUTTON") {
			throw new Error("invalid jquery object. use <button /> tag.");
...
export class VMCheckBox extends VMUI {
	constructor(
		protected _obj: any,
		targetClass: any,
		protected targetPropKey: string,
		protected targetPropFlag: number,
		protected isInverse
	) {
		super(_obj, targetClass);
...
export class VMVariableInput extends VMUI {
	...
	protected setTarget() {
		...
		if (
			this._target[this.targetPropKey] == undefined ||
			!TypeChecker.isNumber(this._target[this.targetPropKey])
		) {
...
export class VMTextInput extends VMUI {
	...
	protected setTarget() {
		if (
			this._target[this.targetPropKey] == undefined ||
			!TypeChecker.isString(this._target[this.targetPropKey])
		) {
