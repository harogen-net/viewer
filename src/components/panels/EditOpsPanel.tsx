...
export const EditOpsPanel: FC = () => {
  const { undo, redo, applySlideChangeLive, recordHistory, snapshot } =
    useDocumentMutation();
  ...
  const textLayer =
    selectedLayer?.type === "text" ? (selectedLayer as { text: string }) : null;
  ...
  const handleTextLiveChange = (value: string) => {
    ...
    applySlideChangeLive((s) =>
      updateTextLayerOp(s, layerIndex, { text: value }),
    );
  };
