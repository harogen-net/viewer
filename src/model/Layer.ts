...
export class Layer {
	...
	public scaleBy(scaleX: number, scaleY: number = NaN): void {
		...
		this.dispatchEvent(
			new PropertyEvent(PropertyEvent.UPDATE, this, PropFlags.SCALE_X | PropFlags.SCALE_Y)
		);
	}
	...
	public get y(): number {
		return this._originHeight / 2 + this._transY;
	}
	...
	public get width() {
		return this._originWidth * this._scaleX;
	}
	public get height() {
		return this._originHeight * this._scaleY;
	}
	...
	public get matrix(): number[] {
		var matrix = Matrix4.identity()
			.translate(this._transX, this._transY, 0)
			.rotateZ((this._rotation * Math.PI) / 180)
			.scale(this._scaleX * (this._mirrorH ? -1 : 1), this._scaleY * (this._mirrorV ? -1 : 1), 1);
		return [
			matrix.values[0],
			matrix.values[1],
			matrix.values[4],
